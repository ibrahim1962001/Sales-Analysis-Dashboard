...
export const GoogleSheetsPicker: React.FC<Props> = ({ onFile, lang = 'ar' }) => {
  ...
  /* ── fetch sheet as CSV ──────────────────────────────── */
  const fetchSheet = async (doc: { id: string; name: string }, token: string) => {
    setStatus('loading');
    const name = doc.name;
    setMsg(lang === 'ar' ? `جاري تحميل "${name}"…` : `Loading "${name}"…`);

    try {
      // Export the first sheet as CSV via Sheets export endpoint
      ...
      if (!res.ok) {
        // Fallback: try gapi.client.sheets values.get (needs sheets scope)
  ...
  const done = (file: File, name: string) => {
    setStatus('success');
    setMsg(lang === 'ar' ? `✅ تم تحميل "${name}" بنجاح!` : `✅ "${name}" loaded!`);
    setTimeout(() => { setStatus('idle'); setMsg(''); onFile(file); }, 1400);
  };
